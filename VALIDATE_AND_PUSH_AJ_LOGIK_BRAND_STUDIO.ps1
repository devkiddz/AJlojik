...
Write-Host "`nAJ Logik Brand Studio release validation" -ForegroundColor Cyan
...
Write-Host "Brand Studio route: /admin/brands"
...
git reset -- APPLY_AJ_LOGIK_BRAND_STUDIO_RELEASE.md PACKAGE_SHA256.txt VALIDATE_AND_PUSH_AJ_LOGIK_BRAND_STUDIO.ps1 2>$null

git diff --cached --quiet
$hasStagedChanges = $LASTEXITCODE -ne 0

if ($hasStagedChanges) {
  git commit -m "feat: add Brand Studio and stabilize product media updates"

...
function publicationHelper(status: 'PRIVATE' | 'PENDING_REVIEW' | 'APPROVED' | 'REJECTED') {
  switch (status) {
    case 'PENDING_REVIEW':
    ...
    try {
      ...
      for (const item of list.items) {
        ...
        await addToCart({
          ...
          quantity: Math.min(item.quantity, item.variant.stockLeft)
        });
        added += 1;
      }
      ...
      if (added === 0) {
        feedback.warning({
          ...
          description: 'No available list items could be added to the cart.',
          groupKey: `shopping-list:${list.id}:cart`
        });
      } else {
        feedback.success({
          title: 'Shopping list added to cart',
          description: `${added} ${added === 1 ? 'product is' : 'products are'} now in your cart.`,
          groupKey: `shopping-list:${list.id}:cart`
        });
      }
    } catch (cause) {
      feedback.error({
        title: 'List could not be added to cart',
        description: cause instanceof Error ? cause.message : 'Unable to add this list to cart.',
        groupKey: `shopping-list:${list.id}:cart`
      });
    } finally {
      setAddingAll(false);
    ...
    return (
      <main className="mx-auto grid min-h-[70vh] max-w-3xl place-items-center px-4 py-10 text-center">
        <div>
          ...
          <h1 className="mt-4 text-2xl font-semibold">Shopping list not found</h1>
    ...
    <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
      ...
      <section className="mt-6 overflow-hidden rounded-[2rem] border bg-card">
        <div className="relative p-6 sm:p-8">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              ...
              <h1 className="mt-5 text-3xl font-semibold tracking-tight sm:text-5xl">{list.name}</h1>
              <p className="mt-3 text-sm leading-6 text-muted-foreground sm:text-base">
                {list.description ?? 'A personal shopping plan that stays current with product pricing and availability.'}
              </p>
              <div className="mt-4 flex flex-col items-start gap-2 sm:flex-row sm:items-center">
                <ShoppingListPublicationToggle list={list} />
                <p className="max-w-xl text-xs leading-5 text-muted-foreground">
                  {publicationHelper(list.publicationStatus)}
                </p>
              </div>
              {list.publicationStatus === 'REJECTED' && list.publicationReviewNote ? (
                <p className="mt-4 rounded-2xl border border-rose-500/20 bg-rose-500/5 px-4 py-3 text-sm leading-6 text-rose-700 dark:text-rose-300">
                  <strong>Admin note:</strong> {list.publicationReviewNote}
                </p>
              ) : null}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                disabled={addingAll || !list.items.length}
                ...
                {addingAll ? <LoaderCircle className="size-4 animate-spin" /> : <ShoppingBag className="size-4" />}
                Add list to cart
              </button>
            </div>
          </div>
        ...
        <div className="grid border-t sm:grid-cols-3">
          ...
          <div className="border-t p-5 sm:border-r sm:border-t-0">
            <p className="text-2xl font-semibold">{list.totalQuantity}</p>
            <p className="text-sm text-muted-foreground">Planned quantity</p>
          </div>
          <div className="border-t p-5 sm:border-t-0">
            <p className="text-2xl font-semibold">{formatCurrency(list.totalValue)}</p>
      ...
      <section className="mt-8">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">List contents</p>
            <h2 className="mt-1 text-2xl font-semibold tracking-tight">Your planned products</h2>
          </div>
          <Link
            href={`/store?view=grid&shoppingList=${encodeURIComponent(list.id)}`}
        ...
        {list.items.length ? (
          <div className="mt-5 space-y-3">
            {list.items.map(item => (
              <ShoppingListItemRow
                ...
                item={item}
                busy={mutating}
                onQuantity={quantity => updateItem(list.id, item.id, { quantity })}
          ...
          <div className="mt-5 grid min-h-72 place-items-center rounded-3xl border border-dashed bg-muted/10 p-8 text-center">
            <div>
              ...
              <h3 className="mt-4 font-semibold">This list is ready</h3>
              <p className="mt-2 max-w-md text-sm leading-6 text-muted-foreground">
                Browse the Store, open a product menu and choose “Add to Shopping List”.
              </p>
              <Link
                href={`/store?view=grid&shoppingList=${encodeURIComponent(list.id)}`}
                className="mt-5 inline-flex h-10 items-center gap-2 rounded-xl bg-foreground px-4 text-sm font-semibold text-background">
...
function ShoppingListItemRow({
  ...
  busy: boolean;
  onQuantity: (quantity: number) => Promise<unknown>;
  ...
  return (
    <article className="flex gap-4 rounded-2xl border bg-card p-3 sm:items-center sm:p-4">
      <div className="relative size-24 shrink-0 overflow-hidden rounded-xl bg-muted sm:size-28">
        <Image src={image} alt={item.product.name} fill sizes="112px" className="object-cover" />
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            ...
            <h3 className="mt-1 line-clamp-2 font-semibold">{item.product.name}</h3>
            <p className="mt-1 text-sm text-muted-foreground">
              {item.variant?.label ?? 'Select a variant from the product experience'}
            </p>
          </div>
          <div className="shrink-0 sm:text-right">
            <p className="font-semibold">{formatCurrency(price * item.quantity)}</p>
            <p className={`mt-1 text-xs ${unavailable ? 'text-destructive' : 'text-muted-foreground'}`}>
              {unavailable ? 'Currently unavailable' : `${item.variant?.stockLeft ?? 0} available`}
            </p>
          </div>
        </div>
        <div className="mt-4 flex items-center justify-between gap-3 border-t pt-3">
          <div className="inline-flex items-center rounded-full border bg-background">
            <button
              type="button"
              disabled={busy}
              onClick={() => void onQuantity(Math.max(1, item.quantity - 1))}
            ...
            <button
              type="button"
              disabled={busy || unavailable || item.quantity >= (item.variant?.stockLeft ?? 0)}

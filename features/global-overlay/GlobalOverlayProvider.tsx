...
const sizeClasses: Record<GlobalOverlayManagerSize, string> = {
  sm: 'w-[min(96vw,30rem)]',
...
function stageClasses(variant: GlobalOverlayVariant) {
  switch (variant) {
    case 'panel':
...
function surfaceClasses(variant: GlobalOverlayVariant, size: GlobalOverlayManagerSize) {
  switch (variant) {
    case 'workspace':
      return [
    ...
    case 'sheet':
      return ['max-h-[92dvh] w-full max-w-none rounded-t-[var(--overlay-radius)]'];

    case 'fullscreen':
      return ['h-dvh w-screen max-w-none rounded-none'];

    default:
      return ['max-h-[min(92dvh,68rem)] rounded-[var(--overlay-radius)]', sizeClasses[size]];
  }
}

export function GlobalOverlayProvider({ children }: { children: ReactNode }) {
  const [stack, setStack] = useState<GlobalOverlayEntry[]>([]);

  const [registeredDialogs, setRegisteredDialogs] = useState<GlobalOverlayRegistration[]>([]);

  const sequenceRef = useRef(0);

  const activeOverlay = stack.at(-1) ?? null;

  const createEntry = useCallback((config: GlobalOverlayConfig): GlobalOverlayEntry => {
    sequenceRef.current += 1;

    return {
      ...config,

      id: config.id ?? `global-overlay-${sequenceRef.current}`,
  ...
  const registerOverlay = useCallback((registration: GlobalOverlayRegistration) => {
    setRegisteredDialogs(current => [...current.filter(item => item.id !== registration.id), registration]);
  }, []);

  const unregisterOverlay = useCallback((overlayId: string) => {
    setRegisteredDialogs(current => current.filter(item => item.id !== overlayId));
  }, []);

  const bringOverlayToFront = useCallback((overlayId: string) => {
    setRegisteredDialogs(current => {
      const existing = current.find(item => item.id === overlayId);

      if (!existing || current.at(-1)?.id === overlayId) {
        return current;
      }

      return [...current.filter(item => item.id !== overlayId), existing];
    });
  }, []);

  const topOverlayId = activeOverlay?.id ?? registeredDialogs.at(-1)?.id ?? null;

  const getOverlayLayer = useCallback(
    (overlayId: string) =>
      Math.max(
        0,
        registeredDialogs.findIndex(item => item.id === overlayId)
      ),
    [registeredDialogs]
  );

  const isTopOverlay = useCallback(
    (overlayId: string) => !activeOverlay && topOverlayId === overlayId,
    [activeOverlay, topOverlayId]
  );

  const hasOpenOverlay = Boolean(activeOverlay) || registeredDialogs.length > 0;
  ...
  const activeVariant = activeOverlay?.variant ?? 'dialog';

  const activeSize = activeOverlay?.size ?? 'md';

  const dismissible = activeOverlay?.dismissible ?? true;
      ...
      <DialogPrimitive.Root
        ...
        {activeOverlay ? (
          <DialogPrimitive.Portal>
            <DialogPrimitive.Backdrop className="fixed inset-0 z-[400] bg-black/55 backdrop-blur-[3px] duration-150 data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0" />

            <div
              data-global-overlay-stage
              className={cn(
                'pointer-events-none fixed inset-0 z-[420] flex min-h-0 min-w-0 overflow-hidden',
                activeVariant === 'workspace' ||
                  activeVariant === 'fullscreen' ||
                  activeVariant === 'panel' ||
                  activeVariant === 'sheet'
                  ? 'p-0'
                  : 'p-3 sm:p-6',
                activeVariant === 'workspace' ? 'sm:p-5' : null,
                stageClasses(activeVariant)
              )}>
              <DialogPrimitive.Popup
                key={activeOverlay.id}
                data-global-overlay-surface
                data-overlay-id={activeOverlay.id}
                className={cn(
                  ...
                  surfaceClasses(activeVariant, activeSize),
                ...
                <header className="relative z-10 border-b border-border/60 bg-popover/92 px-4 py-3.5 backdrop-blur-xl sm:px-6 sm:py-4">
                  <div className="flex min-w-0 items-start gap-3">
                    ...
                    <button
                      ...
                      aria-label={activeOverlay.closeLabel ?? 'Close overlay'}>
                      <X className="size-4" />
                    </button>
                  </div>
                </header>
...
export function useGlobalOverlay() {
  ...
  if (!context) {
    throw new Error('useGlobalOverlay must be used inside GlobalOverlayProvider.');
  }

...
export type JourneyCardTone = 'slate' | 'rose' | 'violet' | 'emerald' | 'amber';
...
export function JourneyCardShell({
  ...
  return (
    <Link
      ...
      className={cn(
        'group relative min-h-64 w-[var(--dashboard-rail-card-width)] min-w-0 shrink-0 snap-start overflow-hidden rounded-2xl border border-t-4 border-border/60 bg-card shadow-sm transition duration-300',
        'hover:-translate-y-1 hover:border-x-primary/25 hover:border-b-primary/25 hover:shadow-lg',
        styles.border,
        className
      )}>
      <div
      ...
      <div className="relative flex h-full min-w-0 flex-col p-3">
        <header className="flex items-start justify-between gap-2">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <div className="min-w-0">
              ...
              <span
                className={cn(
                  'mt-1 inline-flex w-fit items-center rounded-md px-1.5 py-0.5 text-[10px] font-bold',
                  styles.icon
                )}>
                {code}
              </span>
        ...
        <div className="mt-3 h-40 min-h-40 min-w-0 overflow-hidden">{children}</div>

        <footer className="mt-auto flex min-w-0 items-end justify-between gap-2 pt-3">
          <div className="min-w-0">
            <div className={cn('text-2xl font-bold leading-none', styles.metric)}>{metric}</div>

            <p className="mt-1 truncate text-xs text-muted-foreground">{supportingLabel}</p>
          </div>

          <span className="shrink-0 text-[10px] font-semibold text-muted-foreground transition group-hover:text-foreground sm:text-[11px]">
            {actionLabel}
          </span>
        </footer>
      </div>

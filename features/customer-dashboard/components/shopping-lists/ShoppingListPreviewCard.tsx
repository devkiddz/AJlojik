...
import { ArrowUpRight, LockKeyhole, Share2 } from 'lucide-react';
...
export function ShoppingListPreviewCard({ list }: ShoppingListPreviewCardProps) {
  ...
  return (
    <Link href={`/account/lists/${list.id}`} className="group block min-w-[17rem] max-w-[20rem] flex-1">
      <article className="h-full overflow-hidden rounded-2xl border bg-card transition duration-200 hover:-translate-y-0.5 hover:border-foreground/20 hover:shadow-lg">
        <div className="grid h-40 grid-cols-2 gap-px overflow-hidden bg-border">
          {previewItems.length > 0 ? (
            ...
            <div className="col-span-2 flex items-center justify-center bg-muted">
          ...
          {previewItems.length === 1 && <div className="bg-muted/60" />}
          ...
          {previewItems.length === 3 && <div className="bg-muted/60" />}
        </div>
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                ...
                <span>{list.visibility === 'SHARED' ? 'Shared list' : 'Private list'}</span>
              </div>

              <h3 className="mt-2 truncate text-base font-semibold tracking-tight">{list.name}</h3>
            </div>

            <div className="flex size-8 shrink-0 items-center justify-center rounded-full border bg-background transition-colors group-hover:bg-foreground group-hover:text-background">

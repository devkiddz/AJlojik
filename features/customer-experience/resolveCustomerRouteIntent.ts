...
import { resolveCustomerSurface } from './customerExperienceRoutes';
...
function buildRoute(pathname: string, searchParams: SearchParamsReader): string {
  const query = searchParams.toString();
  return query ? `${pathname}?${query}` : pathname;
}

function routeTitle(surface: string): string {
  switch (surface) {
    case 'home':
...
export function resolveCustomerRouteIntent(
  ...
  const surface = resolveCustomerSurface(pathname);
  ...
  if (pathname === '/store' || pathname.startsWith('/store/')) {
    ...
    if (productId) {
      return {
        id: `route:${route}:product:${productId}`,
        type: 'product',
        source: 'route',
        targetId: productId,
        categorySlug,
    ...
    if (collectionId) {
      return {
        id: `route:${route}:collection:${collectionId}`,
        type: 'collection',
        source: 'route',
    ...
    if (promotionId) {
      return {
        id: `route:${route}:promotion:${promotionId}`,
        type: 'promotion',
        source: 'route',
    ...
    return {
      id: `route:${route}:store:${categorySlug}`,
  ...
  if (pathname.startsWith('/promos/')) {
    const promotionId = decodeURIComponent(pathname.slice('/promos/'.length));

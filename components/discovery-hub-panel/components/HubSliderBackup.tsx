...
export default function HubSlider({ items, autoSlide = false, variant = 'strip' }: HubSliderProps) {
  ...
  if (variant === 'hero' && activeItem) {
    return (
      <div className="mt-4">
        <div className="relative overflow-hidden rounded-xl border border-primary/10 bg-background shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
              ...
              {variant === 'grid' && (
                <>
                  ...
                  {formatPrice(item.price) && (
                    <p className="mt-1 text-[12px] font-bold text-primary/80">{formatPrice(item.price)}</p>
                  )}
                </>
              )}
            </div>
    ...
    <div className="mt-4 flex gap-3 overflow-x-auto pb-1 scrollbar-none">
      {safeItems.map(item => (
        <div key={item.id} className="w-24 shrink-0">
          ...
          {formatPrice(item.price) && (
            <p className="text-[12px] font-semibold text-primary/45">{formatPrice(item.price)}</p>
          )}
        </div>
      ))}
    </div>

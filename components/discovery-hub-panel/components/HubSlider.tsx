...
export default function HubSlider({ items, autoSlide = false, variant = 'strip' }: HubSliderProps) {
  ...
  if (variant === 'hero' && activeItem) {
    return (
      <div className="mt-4">
        <div className="relative overflow-hidden rounded-2xl border border-primary/10 bg-background shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
          <div className="relative h-56">
            ...
            <div className="absolute inset-x-0 bottom-0 p-4">
              {activeItem.badge && (
                <span className="mb-2 inline-flex rounded-full bg-primary/15 px-2.5 py-1 text-[10px] font-semibold text-primary">
                  {activeItem.badge}
                </span>
              )}
              ...
              {activeItem.subtitle && (
                <p className="mt-1 line-clamp-2 text-xs text-white/65">{activeItem.subtitle}</p>
              )}
            </div>
          </div>
        </div>
    ...
    return (
      <div className="mt-4 grid grid-cols-3 gap-3">
        {safeItems.slice(0, 4).map(item => (
          <div
            key={item.id}
            className="overflow-hidden rounded-2xl border border-primary/10 bg-background/45 shadow-[0_12px_35px_rgba(0,0,0,0.22)]">
            <div className="relative h-40">
              <Image src={item.image} alt={item.title} fill className="object-cover" />

              {item.badge && (
                <span className="absolute left-2 top-2 rounded-full bg-black/50 px-2 py-0.5 text-[9px] font-semibold text-white">
                  {item.badge}
                </span>
              )}
            </div>
            ...
            <div className="p-2.5">
              ...
              {variant === 'grid' && (
                <>
                  {item.subtitle && (
                    <p className="mt-0.5 line-clamp-1 text-[10px] text-primary/50">{item.subtitle}</p>
                  )}

                  {formatPrice(item.price) && (
                    <p className="mt-1 text-[11px] font-bold text-primary/80">{formatPrice(item.price)}</p>
                  )}
                </>
              )}
            </div>
    ...
    <div className="mt-4 flex gap-3 overflow-x-auto pb-1 scrollbar-none">
      {safeItems.map(item => (
        <div key={item.id} className="w-24 shrink-0">
          ...
          {formatPrice(item.price) && (
            <p className="text-[10px] font-semibold text-primary/45">{formatPrice(item.price)}</p>
          )}
        </div>
      ))}
    </div>

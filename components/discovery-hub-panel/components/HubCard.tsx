...
const statusStyles = {
  idle: 'bg-background/10 text-primary',
...
export default function HubCard({ widget }: HubCardProps) {
  ...
  return (
    <article
      className={cn(
        'overflow-hidden rounded-2xl border border-primary/10 bg-card/40 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_20px_60px_rgba(0,0,0,0.25)] transition hover:border-primary/20 hover:bg-card/60',
      ...
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        {widget.badge && (
          <span className="rounded-full bg-background/40 px-2.5 py-1 text-[10px] font-semibold text-accent">
            {widget.badge}
          </span>
        )}
      </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              {widget.stats.map(stat => (
                <div key={stat.label} className="rounded-xl bg-background/40 p-3 text-center">
                  <p className="text-[10px] font-medium text-amber-300/80">{stat.label}</p>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          {widget.stats.map(stat => (
            <div key={stat.label} className="rounded-xl bg-background/35 p-3">
              <p className="text-[10px] font-medium text-accent">{stat.label}</p>
              <p className="mt-1 text-sm font-bold text-accent">{stat.value}</p>
              {stat.helper && <p className="mt-0.5 text-[10px] text-primary/45">{stat.helper}</p>}
            </div>
          ))}
        </div>
      ) : null}
      ...
      {widget.progress && (
        <div className="mt-4 rounded-xl bg-background/35 p-3">
          ...
          {widget.progress.helper && (
            <p className="mt-2 text-[10px] text-primary/45">{widget.progress.helper}</p>
          )}
        </div>
      )}
          ...
          {widget.timeline.map(item => (
            <div key={item.id} className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                {item.time && <p className="mt-0.5 text-[10px] text-primary/45">{item.time}</p>}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          {widget.conditions.map(condition => (
            <div key={condition.label} className="rounded-xl bg-background/35 p-2">
              <p className="text-[10px] text-accent">{condition.label}</p>
      ...
      {widget.insight && (
        <p className="mt-4 rounded-xl border border-primary/10 bg-background/40 p-3 text-xs leading-5 text-primary/70">
          {widget.insight}
        </p>
      )}

      {widget.action && (
        <button className="mt-4 inline-flex items-center gap-2 rounded-full border border-primary/10 bg-background/50 px-4 py-2 text-xs font-semibold text-primary transition hover:bg-primary hover:text-background">
          {widget.action.label}

...
export default function CategoryPanel({ activeSlug }: CategoryPanelProps) {
  ...
  // Track which category item is currently being hovered
  ...
  return (
    // On mouse leave of the entire panel area, close the subcategories flyout
    <motion.div
            ...
            <div key={cat.id} onMouseEnter={() => setHoveredCategory(cat)} className="relative group">
              <Link href={`/?category=${cat.slug}`}>
                <motion.div
                  ...
                  {/* Tiny indicator chevron like Amazon's main directory */}
                  {!collapsed && cat.subcategories && (
                    <ChevronRight
            ...
            <div className="flex flex-col gap-1 overflow-y-auto">
              {hoveredCategory.subcategories.map((sub: any) => (
                <Link
                  ...
                  onClick={() => setHoveredCategory(null)} // Closes the panel immediately on click
